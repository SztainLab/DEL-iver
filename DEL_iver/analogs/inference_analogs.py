...
def inference_analog_moles(ddr, output_prefix):
    ...
    input_in = ddr.cache._get_output_path(CacheNames.ANALOGS, "similar", prefix=output_prefix)
    parquet_ans = pq.ParquetFile(input_in)
    testdf = parquet_ans.read().to_pandas()

    bbs_to_fings = ddr.cache._get_output_path(CacheNames.ANALOGS, "fingerprints", prefix=output_prefix)
...
def inference_analog_fullmoles(ddr, output_prefix):
    """
    Test the model trained in train_model() on the full molecule analog test set
    ...
    input_in = ddr.cache._get_output_path(CacheNames.ANALOGS_FULL, "similar", prefix=output_prefix)
    parquet_ans = pq.ParquetFile(input_in)
    testdf = parquet_ans.read().to_pandas()

    bbs_to_fings = ddr.cache._get_output_path(CacheNames.ANALOGS_FULL, "fingerprints", prefix=output_prefix)
    ...
    target_test_indices = target_test[['analog']].to_numpy()
