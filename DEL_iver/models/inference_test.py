...
def inference_default(ddr, output_prefix):
    """
    ...
    model_path = ddr.cache._get_output_path(CacheNames.MODELS, "model_default", prefix=output_prefix, ext=".pth")
    ...
    output_out = ddr.cache._get_output_path(CacheNames.MODELS, "predictions_default", prefix=output_prefix)
    ...
    auroc_out = ddr.cache._get_output_path(CacheNames.MODELS, "auroc_plot_default", prefix=output_prefix, ext=".png")
    pr_out    = ddr.cache._get_output_path(CacheNames.MODELS, "pr_plot_default",    prefix=output_prefix, ext=".png")
    ...
    plt.savefig(f"{auroc_out}_default.png")
    print(f'auroc plot saved to {auroc_out}_default.png')
    ...
    plt.legend(loc="lower right", fancybox=True, framealpha=1.0)
    plt.savefig(f"{pr_out}_default.png")
    print(f'precision recall saved to {pr_out}_default.png')


def inference_invariant(ddr, output_prefix):
    """
    Test the model trained in train_model() on the test set also generated in train_model(). (invariant model)
    ...
    model_path = ddr.cache._get_output_path(CacheNames.MODELS, "model_invariant", prefix=output_prefix, ext=".pth")
    ...
    output_out = ddr.cache._get_output_path(CacheNames.MODELS, "predictions_invariant", prefix=output_prefix)
    pq.write_table(pa.Table.from_pandas(predictions_df), output_out)
    ...
    auroc_out = ddr.cache._get_output_path(CacheNames.MODELS, "auroc_plot_invariant", prefix=output_prefix, ext=".png")
    pr_out    = ddr.cache._get_output_path(CacheNames.MODELS, "pr_plot_invariant",    prefix=output_prefix, ext=".png")
    ...
    plt.title(f'{output_prefix} Test Performance (AUROC)')
    plt.legend(loc="lower right", fancybox=True, framealpha=1.0)
    plt.savefig(f"{auroc_out}_invariant.png")
    print(f'auroc plot saved to {auroc_out}_invariant.png')
    ...
    plt.title(f'{output_prefix} Test Performance (PR)')
    plt.legend(loc="lower right", fancybox=True, framealpha=1.0)
    plt.savefig(f"{pr_out}_invariant.png")
    print(f'precision recall saved to {pr_out}_invariant.png')
    
def inference_fullmole(ddr, output_prefix):
    """
    Test the model trained in train_model() on the test set also generated in train_model(). (fullmolecule model)
    ...
    fps = ddr.cache._get_output_path(CacheNames.SMILESEMBEDDING_FULL, "fingerprints_fullmole", prefix=output_prefix)

    # generate the fp dictionaries
    ...
    target_test_indices = target_test[[cols[0]]].to_numpy()

    model_path = ddr.cache._get_output_path(CacheNames.MODELS, "model_fullmole", prefix=output_prefix, ext=".pth")
    ...
    output_out = ddr.cache._get_output_path(CacheNames.MODELS, "predictions_fullmole", prefix=output_prefix)
    pq.write_table(pa.Table.from_pandas(predictions_df), output_out)
    ...
    auroc_out = ddr.cache._get_output_path(CacheNames.MODELS, "auroc_plot_fullmole", prefix=output_prefix, ext=".png")
    pr_out    = ddr.cache._get_output_path(CacheNames.MODELS, "pr_plot_fullmole",    prefix=output_prefix, ext=".png")
    ...
    plt.title(f'{output_prefix} Test Performance (AUROC)')
    plt.legend(loc="lower right", fancybox=True, framealpha=1.0)
    plt.savefig(f"{auroc_out}_fullmole.png")
    print(f'auroc plot saved to {auroc_out}_fullmole.png')
    ...
    plt.title(f'{output_prefix} Test Performance (PR)')
    plt.legend(loc="lower right", fancybox=True, framealpha=1.0)
    plt.savefig(f"{pr_out}_fullmole.png")
    print(f'precision recall saved to {pr_out}_fullmole.png')